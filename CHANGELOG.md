...
**Petición de Víctor**: cerrar todos los días a las 00:00 las sesiones abiertas (ya lo hacían ambas apps) y, si pasan 72 horas hábiles sin volver a iniciar sesión, exigir un código de verificación enviado al correo registrado. Al construir esta pieza en Control de Pedidos se detectaron, de forma no buscada, dos fallos de seguridad — el segundo, crítico — que Víctor autorizó corregir de inmediato (AskUserQuestion) antes de seguir.

**Contexto — de dónde viene todo esto**:
1. El cierre diario de sesión ya existía en ambas apps (Control de Pedidos y DALI); Víctor confirmó que el comportamiento actual (caducidad al primer acceso tras cambiar el día, no una expulsión activa a las 00:00 en punto) es suficiente.
2. Control de Pedidos ya tenía, desde julio, un mecanismo de verificación por email tras inactividad — pero contaba **días naturales** sin login (`DIAS_VERIFICACION_EMAIL`), no las 72 horas seguidas pospuestas a día hábil que pidió Víctor esta vez. Se sustituyó por `HORAS_VERIFICACION_EMAIL = 72` + la nueva `_es_dia_habil()`: se exige el código en cuanto han pasado 72 horas reales desde `ultimo_login` **y** el momento en que se cumplen cae en día hábil (si cae en fin de semana, se pospone al primer día hábil siguiente) — mismo criterio, y misma función auxiliar en espíritu, que ya se construyó para el aviso visual de catálogo desactualizado en DALI.
3. Al revisar cómo mandaba Control de Pedidos ese código de verificación, se confirmó que lo hacía el **navegador** de quien acaba de hacer login, vía EmailJS con la Public Key — para eso, el backend tenía que devolver el código en la propia respuesta de `/api/login`, visible para cualquiera que mirase esa respuesta (DevTools → Red, o repitiendo la petición a mano). No es tan grave como el punto 4 porque hace falta conocer ya la contraseña correcta, pero rompe el propósito de la verificación por email. Víctor eligió (AskUserQuestion) "Añadir clave privada de EmailJS a Control Pedidos" como solución.
4. **[CRÍTICO]** Al construir esa infraestructura de Private Key, se revisó si el mismo patrón (envío desde el navegador con datos sensibles en la respuesta) aparecía en algún otro sitio — y apareció: `solicitar_reset_password()` (`POST /api/password-reset/solicitar`, endpoint público, sin sesión) devolvía el **enlace real de restablecimiento de contraseña** (con el token válido) directamente en su respuesta JSON, además del HTML con ese mismo enlace embebido en un botón. Cualquiera que conociera (o probara) un username/email ajeno podía leer esa respuesta y quedarse con un enlace de reseteo válido durante 2 horas para ESA cuenta, sin tocar nunca su email real — toma de control de cuenta completa, sin dejar ningún rastro para la víctima. Si el envío por EmailJS fallaba, el fallback era todavía peor: se mostraba el enlace **directamente en pantalla** ("Paso 2b: sin email configurado") como "último recurso". Exactamente el mismo fallo, con el mismo origen, que se encontró y corrigió el mismo día en el repo hermano DALI (`recuperarAcceso`, ver `HISTORIAL.md` v1.52 de ese repo) — reportado a Víctor de inmediato, que autorizó el arreglo ya (AskUserQuestion: "Sí, arréglalo ya").

**Cambio en `app.py`**:
- `HORAS_VERIFICACION_EMAIL = 72` sustituye a `DIAS_VERIFICACION_EMAIL = 3`; nueva `_es_dia_habil(fecha)`. El bloque de `login()` que decidía `dias_inactivo` se reescribe para calcular horas reales transcurridas desde `ultimo_login` y posponer la exigencia a un día hábil.
- Nuevas claves de configuración `emailjs_private_key_1..4` en `config_alertas` (una por cada una de las 4 cuentas EmailJS ya existentes) — vacías por defecto, `ON CONFLICT DO NOTHING` como el resto de claves de ese grupo.
- Nueva función `_enviar_emailjs_servidor()`: envía un correo llamando DIRECTAMENTE a la API REST de EmailJS (`https://api.emailjs.com/api/v1.0/email/send`) con la Private Key (`accessToken`) de la cuenta activa (o la siguiente del ciclo que tenga las 4 credenciales completas, incluida la Private Key) — el navegador no interviene en absoluto. Best-effort: si falla (sin Private Key configurada, cuota agotada, red…) no lanza, solo lo registra en el log — la respuesta pública nunca debe distinguir un envío fallido de uno correcto, ni revelar si la cuenta existe.
- El núcleo de contador/rotación cíclica de `/api/emailjs/registrar-envio` se extrae a `_registrar_envio_emailjs_interno()` para que también lo reutilice `_enviar_emailjs_servidor()` tras cada envío real — mismo contador para los envíos "de navegador" (Public Key) y "de servidor" (Private Key).
- `login()`: el código de verificación ya no viaja en la respuesta JSON (se eliminan `email`/`nombre`/`subject`/`message`/`body_html` de esa respuesta) — se manda con `_enviar_emailjs_servidor()` antes de responder.
- `solicitar_reset_password()` [CRÍTICO]: ya no devuelve `link`/`body_html`/`subject`/`sin_email`/`email`/`nombre` en ningún caso — la respuesta es siempre el mismo mensaje genérico, exista o no el usuario, haya podido enviarse el correo o no. El enlace se manda con `_enviar_emailjs_servidor()`; solo queda registrado en el log del servidor (nunca en una respuesta HTTP).
- `_permite_registrar_envio_no_autenticado()`: de los 3 flujos que documentaba, solo queda `solicitar_usuario_fase2()` (aviso a administradores, sin ningún secreto del destinatario) — los otros dos (código de login, reset de contraseña) ya no pasan por sesión ni por HTTP para registrar su envío.

**Cambio en `templates/index.html`**:
- Admin → EmailJS: nuevo campo "Private Key (envío desde servidor)" en cada una de las 4 tarjetas de cuenta, con el mismo botón 👁/🙈 de mostrar/ocultar ya usado en Usuarios/Login — se guarda solo con el mecanismo genérico ya existente (`saveEmailjsConfig()`, sin cambios).
- `doLogin()`: eliminada la llamada a `_enviarCodigoVerificacion()` (mandaba el código por EmailJS desde el navegador) — el backend ya lo ha enviado al responder.
- `enviarSolicitudReset()` [CRÍTICO]: eliminado el envío de EmailJS desde el navegador y, sobre todo, el fallback que mostraba el enlace real EN PANTALLA cuando ese envío fallaba — ahora solo muestra la confirmación genérica de siempre, en todos los casos. Se elimina el "Paso 2b" del modal (ya no tiene ningún dato que mostrar) y la función `copiarEnlaceReset()`.

**Verificación**: `python3 -c "import ast; ast.parse(...)"` sin errores sobre `app.py`; `node --check` sobre los 8 bloques `<script>` de `index.html` (extraídos con un parser HTML real, no con una expresión regular ingenua) sin errores. Test funcional sobre el CÓDIGO REAL de `_enviar_emailjs_servidor()` (extraído de `app.py` vía `ast`, no retipeado a mano) con la API de EmailJS simulada: sin ninguna cuenta con Private Key → no envía y no lanza; cuenta activa completa → payload correcto (`service_id`/`template_id`/`user_id`=Public Key/`accessToken`=Private Key/`template_params` con `to_email`/`subject`/`message`/`reply_to`) y contador registrado; cuenta activa incompleta → recorre el ciclo (1→2→3→4) hasta encontrar una cuenta con Private Key; EmailJS responde error HTTP → no envía, no lanza, no registra el contador; excepción de red → no lanza. Inspección estática (vía `ast`) de todos los `jsonify()` de `login()` y `solicitar_reset_password()`: confirmado que ninguno de sus casos devuelve `link`/`body_html`/`subject`/`email`. Umbral de 72h hábiles verificado con un test standalone replicando la lógica contra fechas concretas (60h insuficiente; 72h en sábado pospuesto a lunes; viernes 10:00+72h=lunes inmediato; 1h antes del umbral, correctamente sin exigir; sin `ultimo_login`, siempre exige).

**Revisión de otros documentos (norma 5)**: `README.md` sí (versión actual). `PENDIENTES.md` sí — **corrección tras auditoría posterior**: se había marcado "no aplica" por error; añadida entrada nueva ("Rellenar la Private Key de EmailJS") con el mismo patrón que el resto de entregas que quedan pendientes de una acción de Víctor en producción. `GUIA_DESPLIEGUE.md` sí — **corrección tras auditoría posterior**: el paso 2 (EmailJS) seguía diciendo "el email se gestiona íntegramente desde el frontend", ya no es del todo cierto para estos dos correos — añadido el aviso y el paso 5 con la Private Key. `INSTRUCCIONES_RESTAURACION.md` — no aplica. `docs/hallazgo-seguridad-princess.md` — no existe en este repo (ver nota de norma 5 en `docs/HISTORIAL_CAMBIOS.md`).

**Nota sobre el número de versión**: esta entrega se construyó, por error, sobre una copia local desactualizada del proyecto (una versión anterior a la v12.32.54 real, que ya incorporaba el cambio de "Proveedor asignable a mano en fase de cotización" de Víctor). Al detectarlo (Víctor avisó de la colisión de número de versión), se rehicieron los mismos cambios directamente sobre el `v12.32.54` real que Víctor tiene desplegado, y se entrega aquí como `v12.32.55` — sin perder ni pisar el cambio de "Proveedor" de la v12.32.54.

**Sigue pendiente**: que Víctor rellene al menos una Private Key de EmailJS (Admin → EmailJS) para que el código de verificación de login y el nuevo enlace de restablecimiento de contraseña puedan enviarse — hasta entonces, ambos flujos funcionan en modo best-effort (quedan registrados en el log del servidor, pero el correo no llega) sin que la respuesta pública lo refleje, por diseño.
...
# v12.32.54 — 10 septiembre 2026

🏷️ El Proveedor vuelve a poder elegirse a mano en pedidos que todavía no tienen el PDF oficial (fase de cotización) — el PDF, cuando llega, sigue mandando siempre

**Petición de Víctor**: "como el proveedor ahora se asigna automáticamente al subir el PDF del pedido, para poder grabar los pedidos como PENDIENTE DE COTIZACION, necesito que tengan un proveedor asignado para seguimiento del presupuesto, reclamación del mismo etc. ¿Cómo lo podemos gestionar? ¿Qué ideas me das?" — propuesta aceptada tal cual ("sí me parece buena idea"): reactivar el buscador manual de proveedor, pero solo mientras el pedido no tenga adjuntado el PDF oficial; en cuanto se adjunta, el proveedor que traiga ese PDF sustituye siempre al asignado a mano, con un aviso si no coincide.

**Contexto**: desde v12.32.35, el campo Proveedor del formulario de pedido pasó a ser de solo lectura, rellenado exclusivamente al leer el PDF de «Nº Pedido (DALI/SAP)» (`_resolver_proveedor_pdf_oficial`) — correcto para un pedido que ya tiene ese PDF, pero PENDIENTE COTIZACIÓN es justo la fase ANTERIOR a que exista: se está pidiendo presupuesto, sin pedido de SAP ni PDF todavía, así que el proveedor se quedaba sin asignar hasta que el pedido avanzara. Esto dejaba dos consecuencias que Víctor quería resolver: sin proveedor no había a quién hacer seguimiento de consumo/presupuesto, y la reclamación automática de cotización pendiente (`_encolar_reclamacion_proveedor`, extendida a PENDIENTE COTIZACIÓN en v12.23.6) no tenía a quién reclamar y avisaba en su lugar, únicamente, a los compradores del hotel (`_encolar_aviso_cotizacion_sin_proveedor`) — comportamiento de reserva que sigue existiendo tal cual para cuando de verdad no hay proveedor, pero que dejaba de tener sentido en cuanto Víctor SÍ conoce el proveedor al pedir presupuesto y quiere registrarlo.

El código del buscador manual (`buscarProveedor`/`seleccionarProveedor`, en `templates/index.html`) nunca se había borrado al desactivarlo en v12.32.35 — el comentario de entonces decía explícitamente "no se ha borrado por si se recupera en algún caso especial". Este es ese caso.

**Cambios en `app.py`**:
- `create_pedido()`: `proveedor_id` deja de forzarse siempre a `NULL` al crear — se acepta si viene en `data` y existe en el catálogo de proveedores (validado con una consulta explícita, error 400 si no existe). No hay riesgo de que esto permita dar de alta un pedido ya "enviado al proveedor" sin PDF: esa comprobación (independiente del proveedor) sigue en pie más arriba en la misma función, y en `_validar_pedido_envio_proveedor()` (puntos 1 y 2: `pedido_num` y el propio adjunto `pedido_doc` son obligatorios e independientes de si hay proveedor).
- `update_pedido()`: mismo criterio — si el pedido YA tiene `pedido_num` (que solo llega a tener valor al adjuntar el PDF, es la misma señal que usa el resto de la app para "ya tiene PDF"), el proveedor se preserva sin excepción, exactamente igual que antes de este cambio; si NO lo tiene todavía, se acepta la reasignación manual de `data`, validando de nuevo que el proveedor exista en el catálogo (solo si de verdad cambia, para no repetir la consulta en cada guardado sin tocar el campo).
- `upload_adjunto()` (rama `pedido_doc`): antes de sobrescribir con el proveedor leído del PDF (comportamiento sin cambios — el PDF sigue mandando siempre, igual que con `pedido_num`/`total_pedido`/Departamento/Hotel), se guarda el proveedor que hubiera en ese momento y se compara contra el nuevo — la respuesta añade `proveedor_manual_previo_id`, `proveedor_manual_previo_nombre` y `proveedor_manual_coincide` (`null` si no había ningún proveedor previo que comparar, caso normal sin nada que avisar; `true`/`false` si lo había). Puramente informativo: nunca bloquea ni impide la subida del PDF.

**Cambios en `templates/index.html`**:
- Campo Proveedor: se recupera el contenedor `prov-dropdown` (eliminado del HTML en v12.32.35, aunque el JS que lo usaba nunca se borró) y se reconecta `oninput="buscarProveedor(this.value)"` sobre `p-proveedor-input`.
- Nueva función `_actualizarBloqueoProveedorManual(bloqueado)`: alterna el campo entre solo-lectura (gris, con el mismo placeholder de siempre) y editable (blanco, buscable, con una nota de que es un proveedor "provisional para la fase de cotización"). Se llama con `false` al preparar un pedido nuevo (`resetForm`, nunca tiene PDF todavía), con `!!p.pedido_num` al abrir un pedido existente (`poblarFormulario` — bloqueado si ya tiene PDF, editable si no), y con `true` justo después de aplicar los datos de un PDF recién subido (`subirAdjuntos`, rama `pedido_doc`) — mismo criterio (`pedido_num`) que ya usa el propio backend para lo mismo.
- `subirAdjuntos()`: si `proveedor_manual_coincide === false`, un aviso flotante nuevo señala que el proveedor del PDF ha sustituido al asignado a mano en la cotización, nombrando ambos — además de (sin cambios) el aviso ya existente si el proveedor del PDF no se reconoce en el catálogo.
- Mensaje de `savePedido()` al intentar pasar a ENVIADO AL PROVEEDOR sin proveedor: actualizado para dejar claro que un proveedor asignado a mano en la cotización es solo provisional y no sustituye a la obligación de adjuntar el PDF oficial.

**Verificación**: `python3 -m py_compile app.py` sin errores. `node --check` sobre el contenido de los 8 bloques `<script>` de `index.html` (extraídos con un parser HTML real, no con una expresión regular — una regex ingenua sobre `<script>` se confunde con la cadena literal "<script>" que aparece dentro de un comentario CSS ya existente en el archivo) sin errores. Revisión manual línea a línea de las tres funciones tocadas en `app.py` (`create_pedido`, `update_pedido`, `upload_adjunto`) y de cada punto de `_validar_pedido_envio_proveedor()` para confirmar que ningún camino permite llegar a ENVIADO AL PROVEEDOR sin el PDF real, con o sin proveedor manual de por medio. **No se ha podido probar contra una base de datos PostgreSQL real ni contra datos de producción** — este entorno no tiene acceso a ninguna de las dos; ver `PENDIENTES.md` para la lista de comprobaciones que faltan en producción.

**Revisión de otros documentos (norma 5)**: `README.md` sí (párrafo de "Pedidos" en Funcionalidades, y versión actual). `PENDIENTES.md` sí — nueva entrada, entregado pendiente de prueba en producción (ver más arriba). `GUIA_DESPLIEGUE.md`, `INSTRUCCIONES_RESTAURACION.md`, `docs/hallazgo-seguridad-princess.md` — no aplica (sin menciones relacionadas).

**Entrega**: `app.py`, `templates/index.html`, `README.md`, `PENDIENTES.md`, más este changelog/`docs/HISTORIAL_CAMBIOS.md`. `models.py` y `requirements.txt` no cambian.

**Sigue pendiente**: que Víctor pruebe el flujo completo en producción (crear pedido en PENDIENTE COTIZACIÓN, asignar proveedor a mano, guardar/reabrir, adjuntar después el PDF oficial y comprobar la sustitución/aviso, y confirmar que la reclamación automática de cotización pendiente ya le llega al proveedor) — ver el detalle completo en `PENDIENTES.md`.
...
**Petición de Víctor**, tras una captura de un correo real de "Documentación pendiente — BRIGONSA GROUP 2015 SL": "¿sería posible que los correos que envía EmailJS se puedan contestar a una cuenta indicada? por ejemplo, grabar qué responder a... el correo del comprador".

**Causa**: `_enviarEmailsSistemaPendientes()` (`templates/index.html`) fijaba `reply_to: p.destinatario` para TODA la cola `emails_sistema_pendientes`, sin excepción. Para la mayoría de eventos de esta cola (avisos internos a admins/compradores) da igual — el destinatario es quien debe verlo, responderse a sí mismo es inocuo. Pero para los correos que salen HACIA un proveedor externo (en concreto, el puente con DALI para "Documentación faltante", `evento_codigo='dali_documentacion_faltante'`), esto significaba que si el proveedor le daba a "Responder", el correo volvía a su propia dirección en vez de llegar al comprador que gestionó la solicitud desde DALI — exactamente lo que se ve en la captura de Víctor (`responder a: felipebtoledo@brigonsacanarias.com`, el propio proveedor).

**Corrección**: columna nueva `reply_to` (TEXT, NULL por defecto) en `emails_sistema_pendientes` — cuando viene vacía, el poller mantiene el comportamiento de siempre (`reply_to = destinatario`), así que ningún otro evento de la cola cambia. `POST /api/externo/dali-sap/emails-pendientes` acepta ahora un campo opcional `reply_to` en el body y lo guarda tal cual; DALI (`documentacionController.js`/`controlPedidosEmailBridge.js`, ese repo) lo rellena con `req.user.email` — el mismo email de sesión que ya usa `resolverFirmaAdmin` para firmar el cuerpo del correo, así que no hace falta resolverlo aparte ni añadir ningún campo nuevo en DALI. `GET /api/emails-sistema-pendientes` devuelve la columna nueva en el `RETURNING`, y el poller pasa a usar `reply_to: p.reply_to || p.destinatario`.

**Nota de numeración**: este cambio se preparó en paralelo sobre el ZIP de la v12.32.41, sin conocer todavía que Víctor ya había desplegado en producción una v12.32.42 propia y no relacionada (OCR de PDF de pedido firmado/escaneado + cambio de runtime a Docker, ver entrada anterior). Al recibir el ZIP real ya en v12.32.42, este parche se rehizo sobre esa base para no perder ese trabajo, y pasa a numerarse v12.32.43.

**Verificación**: `python3 -m py_compile app.py` sin errores. Revisión manual del bloque `<script>` tocado en `templates/index.html` (un único objeto literal, sin nada estructural alrededor). No probado en vivo contra producción — pendiente de que Víctor apruebe/rechace una solicitud de documentación real de DALI tras desplegar los dos repos y confirme en Email History (EmailJS) que el "Reply-To" de esa fila es ya el email del comprador, no el del proveedor.

**Revisión de otros documentos (norma 5)**: `GUIA_DESPLIEGUE.md`, `PENDIENTES.md`, `INSTRUCCIONES_RESTAURACION.md` — no aplica (columna nueva vía `ADD COLUMN IF NOT EXISTS`, mismo patrón de siempre, sin paso de despliegue distinto; no toca nada de lo añadido en v12.32.42 para OCR/Docker). `README.md` sí: versión actual.

**Entrega**: `app.py`, `templates/index.html`, más este changelog/`docs/HISTORIAL_CAMBIOS.md`/`README.md`. En el repo de DALI (`dali-sap-articulos-app`, aparte): `backend/src/services/controlPedidosEmailBridge.js`, `backend/src/controllers/documentacionController.js` — sin cambio de versión en ese repo (no toca ningún endpoint propio de cara al usuario, solo un parámetro nuevo opcional en una llamada saliente ya existente).

---

# v12.32.42 — 7 septiembre 2026

📷🔓 El PDF de pedido oficial se acepta también firmado/sellado (escaneado, sin texto) — lectura por OCR como último recurso

**Petición de Víctor**: "Actualmente utilizamos este formato PDF de pedido oficial para el registro de pedidos, necesito que mientras sea el mismo formato y se puedan ver claramente todos los datos/información que se deben leer, se acepte si viene firmado sellado etc.. pero siempre que no tape ninguno de los datos necesarios", adjuntando un ejemplo real de PDF de pedido oficial sin firmar y otro firmado/sellado del mismo formato.

**Hallazgo**: el PDF firmado/sellado que adjuntó Víctor no es el mismo documento con un sello superpuesto sobre su texto — es un documento **totalmente distinto por dentro**: al firmarlo/sellarlo en papel y volver a escanearlo para adjuntarlo, pierde POR COMPLETO su capa de texto. Comprobado con `pypdf.extract_text()` sobre el PDF real: devuelve `""` — el documento son 4 imágenes incrustadas (una de página completa a 1240×1753 y otra a 2048×1776, más el logo y el sello como imágenes aparte), cero caracteres de texto. `_parsear_pdf_pedido_oficial()` no tenía nada sobre lo que buscar el Nº de Pedido ni las líneas de importe con el enfoque de siempre (regex sobre el texto de `pypdf`), así que rechazaba el archivo entero con "El PDF adjuntado no tiene el formato del pedido oficial PRINCESS" — con independencia de que un humano lo leyera perfectamente a simple vista. El PDF sin firmar de ejemplo, en cambio, sí trae su texto embebido de siempre y se seguía leyendo sin problema.

**Corrección (`app.py`)**: `_parsear_pdf_pedido_oficial()` mantiene exactamente el mismo comportamiento de siempre como primera vía (lectura por texto con `pypdf`, instantánea). Solo cuando ese texto sale prácticamente vacío (`_UMBRAL_TEXTO_VACIO_PDF`, 20 caracteres — indicio claro de PDF escaneado, a diferencia de un PDF con texto real que simplemente no es un pedido oficial, caso en el que el OCR no ayudaría y se rechaza directo sin gastar ese coste de CPU) se intenta una segunda vía: **OCR con Tesseract** sobre una imagen renderizada de cada página, nueva función `_ocr_texto_pdf_pedido_oficial()`:
- Renderiza cada página a 300 ppp con `pagina.to_image()` de `pdfplumber` (que a su vez usa `pypdfium2`, ya dependencia del proyecto — no hace falta poppler ni ninguna otra herramienta de rasterizado nueva) y reconoce el texto con `pytesseract` (modelo español, `lang="spa"`).
- El texto reconocido por OCR pasa por los mismos patrones de siempre (`_PATRON_PEDIDO_NUM_OFICIAL`, `_PATRON_IMPORTE_LINEA_OFICIAL`, etc.) — si encuentran Nº de Pedido y al menos una línea de importe, el PDF se acepta igual que uno con texto propio (mismos campos, mismo `dict` de vuelta, con `leido_via_ocr: True` añadido). Si NO los encuentran —p. ej. porque el sello o la firma tapan físicamente esa parte de la página—, se rechaza con un mensaje explícito pidiendo comprobar que el sello/firma no tapen el Nº de Pedido ni las líneas de artículos, cumpliendo la condición que puso Víctor ("siempre que no tape ninguno de los datos necesarios").
- Límite de seguridad `_LIMITE_PAGINAS_OCR_PEDIDO_OFICIAL = 5`: un pedido oficial real es siempre 1 página (visto en todos los ejemplos, incluidos los 10 hoteles de v12.32.40) — un PDF escaneado con más páginas que eso ya no puede ser un pedido oficial real y se rechaza sin gastar CPU intentando el OCR, en línea con la misma cautela de CPU/memoria del plan Free de Render ya documentada para otros lectores de PDF de esta app (ver `GUIA_DESPLIEGUE.md`).
- **Hallazgo secundario, corregido en la misma función**: probado el OCR contra el PDF real firmado, Tesseract confunde a veces los bordes verticales de la tabla con el carácter `"|"` y lo intercala ENTRE los tres números de una línea de artículo (`"40,0000 | 115,00  1.150,00"`), lo que rompía `_PATRON_IMPORTE_LINEA_OFICIAL` (exige solo espacio en blanco entre los números). El documento real nunca trae un `"|"` literal en su texto, así que se elimina siempre del texto reconocido por OCR (nunca del texto de `pypdf`, que es fiable tal cual). Con esta limpieza, el PDF de ejemplo se reconoce correctamente: Nº de Pedido `43192` y línea de importe `1.150,00`.

**Backend (`upload_adjunto`)**: la respuesta del endpoint incluye ahora `leido_via_ocr` (booleano) — puramente informativo, nunca bloquea nada; si se llega hasta ahí es porque el OCR ya encontró los datos obligatorios con garantías.

**Frontend (`templates/index.html`)**: si `leido_via_ocr` viene `true`, se muestra un aviso adicional junto al de siempre ("PDF leído — Nº Pedido…") pidiendo revisar los datos con más atención, ya que vienen de un documento escaneado y no de su texto original.

**Infraestructura — cambio de runtime en Render (`render.yaml`, `Dockerfile` nuevo)**: Tesseract es un binario del sistema operativo, no un paquete de `pip`, y el runtime nativo de Python de Render no permite instalar paquetes de sistema (`apt-get` no está disponible ahí — confirmado en la documentación/comunidad de Render). Por eso el servicio pasa de `runtime: python` a `runtime: docker`, con un `Dockerfile` nuevo que instala `tesseract-ocr` + `tesseract-ocr-spa` (modelo de español; sin él, Tesseract solo traería inglés) antes de instalar las dependencias de `requirements.txt` de siempre. El comando de arranque no cambia de contenido, solo de nombre de campo (`startCommand` → `dockerCommand`, ver comentario en el propio `render.yaml`); se añade también la variable `PORT` explícita, necesaria en `runtime: docker` para que Render sepa a qué puerto interno enrutar (con `runtime: python` se autodetectaba sola). `.dockerignore` nuevo, con cuidado de NO excluir `CHANGELOG.md` ni `docs/` porque `app.py` lee `CHANGELOG.md` en tiempo de ejecución (aviso de nueva versión) y excluirlo habría roto esa funcionalidad en producción.

**`requirements.txt`**: añadido `pytesseract==0.3.13` (el wrapper Python de Tesseract). El binario de Tesseract en sí se instala en el `Dockerfile`, no aquí.

**Verificación**: `python3 -m py_compile app.py` sin errores nuevos (el único aviso, `SyntaxWarning` en un docstring de `backup_restaurar()`, es preexistente y no relacionado con este cambio). Bloque `<script>` de `templates/index.html` donde se añadió el aviso revisado a mano (mismo patrón `if (...) { toast(...); }` que el resto de la función), balance de `<div>` correcto (975/975, sin cambios). Reproducido en un entorno de desarrollo el flujo completo `pdfplumber.to_image()` → `pytesseract.image_to_string()` contra el PDF real firmado de Víctor (con modelo de inglés, por no tener disponible el paquete de idioma español en ese entorno de prueba — el comportamiento con el modelo español de producción debería ser igual o mejor): reconoce correctamente `PEDIDO 00043192` y, tras la limpieza del `"|"`, la línea de importe `1.150,00`. No probado en vivo contra producción (el cambio de runtime a Docker no se puede probar en este entorno de desarrollo, que no tiene acceso a la red para construir la imagen) — **recomendado probar el primer deploy Docker con cuidado** y, si el build de Render instala versiones distintas de las transitivas de `pytesseract`, volver a fijar `requirements.txt` con el log de ese build (ver nota de cabecera del propio archivo).

**Revisión de otros documentos (norma 5)**: `PENDIENTES.md`, `INSTRUCCIONES_RESTAURACION.md`, `docs/hallazgo-seguridad-princess.md` — no aplica. `GUIA_DESPLIEGUE.md` sí: Paso 3 (Render) actualizado para reflejar `runtime: docker` en vez de runtime nativo Python. `README.md` sí: versión actual, stack técnico (Docker) y bullet de Pedidos.
...
**Petición de Víctor**, tras la entrega de v12.32.35 (verificación de Departamento vs. Almacén): "¿otra duda, se verifica que el hotel es el correcto contra el PDF subido? También evitaría registrar un pedido a un hotel incorrecto" — confirmado que hasta ahora el Hotel del pedido nunca se contrastaba con nada del PDF, se quedaba tal cual el usuario lo eligió al crear el pedido, aunque se subiera el PDF de un pedido que en realidad era de otro hotel. Víctor confirmó, a partir del mismo PDF de ejemplo (pedido a PILSA, hotel Maspalomas Tabaiba): "en el pedido pone el nombre completo del hotel, el PDF adjunto por ejemplo corresponde a MT MASPALOMAS & TABAIBA PRINCESS".

**Lectura del PDF (`_parsear_pdf_pedido_oficial()`), verificada con `pypdf.extract_text()` sobre el PDF real de ejemplo**: nueva función `_extraer_hotel_nombre_pdf_oficial()` — a diferencia del Almacén y las Fechas (que van DESPUÉS de "PEDIDO «núm»" en el texto extraído), el nombre completo de HOTEL/CENTRO aparece SIEMPRE justo ANTES: `"...ventas@pilsa.com\nHotel Maspalomas Tabaiba \nPrincess\nPEDIDO 00029875\n..."`. Se toman hasta las 2 líneas de texto inmediatamente anteriores a "PEDIDO «núm»" que parezcan un nombre (letras/números/espacios/puntuación básica, sin "@") y se concatenan — cubre tanto un nombre que cabe en una sola línea como uno partido en dos, como en el PDF de ejemplo ("Hotel Maspalomas Tabaiba" / "Princess"). Es un heurístico best-effort (mismo criterio que el nombre del proveedor, v12.32.35): si no encuentra un candidato claro, devuelve `None` y el hotel del PDF queda sin detectar, sin bloquear el resto del parseo.

**Emparejamiento con el catálogo (`_normalizar_nombre_hotel()`)**: el PDF imprime "Hotel Maspalomas Tabaiba Princess" (con "Hotel" delante y sin el símbolo "&", que pypdf pierde al extraer esa caja) mientras que el catálogo guarda "Maspalomas & Tabaiba Princess" (`hoteles.nombre`, ver `models.py`) — la normalización añade, sobre `_normalizar_texto_generico` ya existente, quitar la palabra "HOTEL" al principio y el símbolo "&", dejando ambos en "MASPALOMAS TABAIBA PRINCESS" para la comparación. Probado directamente contra el PDF real adjuntado por Víctor: coincide con el hotel correcto (MT) y NO coincide con un hotel distinto de prueba.

**Nueva columna `pedidos.hotel_pdf_detectado`**: guarda el nombre leído del PDF, igual que `departamento_pdf_detectado` guarda el Almacén — se rellena al subir el PDF en «Nº Pedido (DALI/SAP)» (`upload_adjunto()`), tanto si coincide con el hotel actual como si no.

**Bloqueo real en `update_pedido()`** (nuevo punto 0d del bloque de validación ENVIADO AL PROVEEDOR, justo después del 0c de Departamento/Almacén): si hay un HOTEL/CENTRO detectado en el PDF, se compara (normalizado) contra el Hotel que el usuario tenga seleccionado AHORA para este cambio de estado — si no coincide, se bloquea el cambio con un mensaje claro hasta corregir el Hotel. Un pedido sin PDF subido, o cuyo PDF no trajo un HOTEL/CENTRO reconocible, no pasa por esta comprobación (mismo criterio exacto que el punto 0c ya existente).

**Aviso inmediato y persistente (`upload_adjunto()` + `templates/index.html`)**: igual que con el Departamento, `upload_adjunto()` devuelve `hotel_pdf`/`hotel_coincide` (comparación informativa contra el hotel que el pedido tiene YA GUARDADO en ese momento) para avisar nada más subir el PDF, y `_mostrarAvisoProveedorPdf()` en el frontend pinta el mismo banner amarillo bajo el campo Proveedor si el Hotel seleccionado no coincide — tanto justo tras subir el PDF como cada vez que se reabre el pedido (mientras el problema siga sin resolver). Nueva función `_revisarHotelTrasCambio()` (mismo patrón que `_revisarDepartamentoTrasCambio()`) recalcula el aviso al cambiar el desplegable de Hotel, enlazada en su `onchange` junto a `poblarSelectDeptos()`. Réplica en JS de la normalización de nombre de hotel (`_normalizarNombreHotelJs`). Bloqueo espejo en el frontend (punto 0d, respuesta inmediata) antes de enviar el cambio de estado — el backend vuelve a comprobarlo igual y es el que de verdad lo impide.

**Verificación**: `python3 -m py_compile app.py` sin errores. El patrón de extracción y la normalización se probaron directamente con `pypdf` contra el PDF real adjuntado por Víctor (pedido a PILSA, hotel Maspalomas Tabaiba) — extrae "Hotel Maspalomas Tabaiba Princess" y coincide correctamente con "Maspalomas & Tabaiba Princess" del catálogo, sin falsos positivos contra otro hotel. Los 8 bloques `<script>` de `templates/index.html` verificados uno a uno con `node --check` (la comprobación de todo el archivo junto da un falso error ya presente en la versión original, por dos apariciones de la palabra "&lt;script&gt;" dentro de comentarios CSS, no de código). Balance de `<div>` correcto (975/975). No probado en vivo contra producción — a confirmar tras desplegar: adjuntar en un pedido dado de alta con el hotel INCORRECTO el PDF oficial de otro hotel (debe avisar y bloquear ENVIADO AL PROVEEDOR hasta corregir el Hotel) y con el hotel correcto (no debe avisar).

**Sin backfill retroactivo**: pedidos creados antes de este cambio (incluidos los que ya tienen su PDF oficial adjuntado) no tienen valor en `hotel_pdf_detectado` — no se ha vuelto a leer ni reparsear ningún adjunto ya guardado, mismo criterio que el resto de columnas `*_pdf_*` desde v12.32.35.
...
**Contexto — por qué esta entrega**: continuación acordada del rediseño, tras Pieza 3 (v12.32.31). Víctor confirmó seguir ("continuamos entonces con el siguiente paso?"). Antes de construir nada se planteó una pregunta de diseño (AskUserQuestion) sobre el riesgo real de esta pieza: a diferencia de "Comparar listado + Albaranes" (cruza por proveedor+importe exacto del listado RESUMIDO, señal fuerte), el cruce que necesita esta pieza para poder prescindir del RESUMIDO solo puede hacerse por proveedor+código de artículo (el Listado de Albaranes de SAP nunca trae el número de pedido) — una señal más débil, que puede ser ambigua. Víctor eligió la opción más segura: "Solo sugerir, nunca cambiar estado solo" — una tabla de auditoría donde él revisa y confirma, pedido a pedido, nunca un cambio automático.

**Qué hace**: nuevo botón "📋 Revisar entregas pendientes (Albaranes)" — para un hotel, calcula de golpe (sin subir ningún PDF, usa lo ya guardado por "Departamentos y líneas" e "Importar Albaranes") qué pedidos pendientes de entrega (ENVIADO AL PROVEEDOR / PENDIENTE COTIZACIÓN / ENTREGA PARCIAL) tienen ya todas o parte de sus líneas cubiertas por algún albarán del mismo proveedor+artículo, y propone ENTREGADO (todas cubiertas) o ENTREGA PARCIAL (solo algunas) para cada uno. El usuario marca qué pedidos aplicar (con casillas, "seleccionar todas" incluido, igual que en "Comparar listado + Albaranes") y solo entonces se actualiza el estado — nunca al calcular.

**Cambio en `app.py`**:
- `_ESTADOS_PENDIENTES_ENTREGA_ALBARAN` (junto a `_ORDEN_ENTREGA_ESTADOS`): los estados de ese mismo diccionario menos `ENTREGADO` — qué pedidos entran en la revisión en bloque.
- `_sugerencias_albaranes_pendientes_hotel(hotel_id)`: versión en bloque de `_sugerencias_albaran_pedido()` (v12.32.19) para todos los pedidos pendientes de un hotel a la vez, precargando de una sola vez todas las líneas de pedidos y todos los albaranes del hotel (nunca N+1 consultas). Por cada pedido, cada línea se considera "cubierta" si tiene un candidato confirmado (PDF de confirmación individual, o número de albarán ya registrado a mano en el pedido) o un único candidato sin ambigüedad con cantidad exacta — igual criterio que la herramienta de un solo pedido. Propone ENTREGADO si TODAS las líneas están cubiertas, ENTREGA PARCIAL si solo ALGUNAS, y nada si ninguna — y descarta cualquier sugerencia que no suponga avanzar el estado actual (`_ORDEN_ENTREGA_ESTADOS`, mismo criterio que ya protegía `_aplicar_coincidencia_albaran()`). Solo lectura, nunca escribe nada.
- `_aplicar_una_sugerencia_albaran_pendiente(db, entry, ...)`: aplica UNA sugerencia — actualiza `estado`, inserta en `historial_estados` con el mismo nombre fijo "Automática — ..." que usa `_aplicar_coincidencia_albaran()` (para que se distinga a simple vista de un cambio manual, aunque lo haya disparado un clic del usuario) y dispara `_notificar_cambio_estado()` con `es_automatico=True` — mismo criterio que esa función hermana. Respeta el mismo guardián que ella: nunca toca un pedido CANCELADO o DENEGADO POR DIRECCION GENERAL, ni retrocede un estado ya más avanzado. A diferencia de `_aplicar_coincidencia_albaran()`, esta primera versión NO toca `entrada_albaran_num` ni `fecha_tramitacion` — decisión deliberada: la señal proveedor+artículo es más débil que proveedor+importe exacto, y registrar aquí un número de albarán concreto a partir de ella podría guardar el albarán equivocado; el número se sigue registrando a mano si hace falta, y se puede ampliar en una pieza siguiente si Víctor lo pide.
- `_aplicar_sugerencias_albaranes_pendientes(db, hotel_id, pedido_ids, ...)`: aplica una lista de pedidos marcados por el usuario — antes de escribir nada, **recalcula las sugerencias en el momento** llamando de nuevo a `_sugerencias_albaranes_pendientes_hotel()` (nunca reutiliza lo que se le mostró antes a la pantalla), para que un pedido que haya dejado de tener sentido entre que se abrió la pantalla y que se pulsó "Aplicar" (otro albarán importado mientras tanto, el pedido cambiado de estado por otra vía) no se aplique con datos ya caducados.
- Dos endpoints nuevos: `GET /api/pedidos/sugerencias-albaranes-pendientes?hotel_id=<id>` (solo lectura) y `POST /api/pedidos/sugerencias-albaranes-pendientes/aplicar` (`{"hotel_id", "pedido_ids": [...]}`, restringido a administradores, igual que el endpoint equivalente de "Comparar listado + Albaranes").

**Cambio en `templates/index.html`**:
- Nuevo botón "📋 Revisar entregas pendientes (Albaranes)" en la barra de Pedidos, junto a "🔍 Sugerencias de Albarán" — visible solo para administradores, mismo criterio que el resto de botones de esta zona.
- Nuevo modal con selector de hotel, tabla de pedidos con casilla por fila + "seleccionar todas" + botón "Aplicar seleccionadas" (mismo patrón que la tabla de coincidencias de "Comparar listado + Albaranes"), detalle de líneas por pedido con los mismos iconos ya usados en "Sugerencias de Albarán" (🔒 confirmado, ✅ coincide exacto, ❔ candidato ambiguo), y aviso explícito de que nunca cambia nada sola.

**Verificación**: `python3 -m py_compile app.py` sin errores; los `<script>` de `templates/index.html` pasan la comprobación de sintaxis sin errores. Probado de extremo a extremo contra un PostgreSQL 16 real, con 6 pedidos construidos a propósito cubriendo los casos posibles: (A) todas las líneas con candidato exacto único → sugiere ENTREGADO; (B) solo una de dos líneas cubierta → sugiere ENTREGA PARCIAL; (C) una línea con 2 candidatos ambiguos, ninguno confirmado → sin sugerencia (no aparece en el resultado); (D) línea con un candidato de cantidad distinta pero confirmado a mano vía `entrada_albaran_num` → cubierta igualmente → ENTREGADO, y comprobado que `entrada_albaran_num`/`fecha_tramitacion` quedan intactos tras aplicar (confirma que esta pieza no los toca); (E) pedido ya CANCELADO con datos que encajarían perfecto → excluido por el filtro de estado, nunca llega a evaluarse; (F) pedido ya en ENTREGA PARCIAL cuya sugerencia también sería ENTREGA PARCIAL → excluido por no suponer avance. También probado: aplicar la misma sugerencia dos veces (la segunda responde "ya está en ese estado", sin duplicar historial ni notificar de nuevo); el endpoint `GET` sin `hotel_id` devuelve 400; el endpoint `POST /aplicar` sin rol admin devuelve 403; y con un `pedido_id` que ya no tiene sugerencia pendiente (recalculado en el momento) responde "sin cambios" en vez de aplicar algo caducado.

**Revisión de otros documentos (norma 5)**: `GUIA_DESPLIEGUE.md`, `INSTRUCCIONES_RESTAURACION.md`, `PENDIENTES.md`, `docs/hallazgo-seguridad-princess.md` — no aplica. `README.md` sí: versión actual.

**Sigue pendiente**: Pieza 5 (unificar los 5 botones actuales del cruce Pedidos↔Albaranes en un flujo de dos pasos) — siguiente entrega. Sigue pendiente también, por separado y sin tocar hoy, la corrección del hueco de egress-tracking en los 4 jobs de subida de PDF (pospuesta a petición explícita de Víctor).

**Entrega**: `app.py`, `templates/index.html`, `README.md`, más este changelog/`docs/HISTORIAL_CAMBIOS.md`. `render.yaml`, `models.py` y `requirements.txt` no cambian — no hay columnas nuevas en esta pieza.
...
**Qué pasó**: tras desplegarse la v12.32.28 (que sí redujo las consultas a la base de datos de casi 4.000 a poco más de una decena), Víctor volvió a subir el mismo listado detallado de 99 páginas / 281 pedidos que ya había fallado antes — y volvió a fallar, con el mismo síntoma ("El job no existe o ha caducado" tras un rato de espera). Esta vez el panel de Eventos de Render fue explícito y no dejó lugar a dudas: **"Instance failed: vvzpz — Ran out of memory (used over 512MB) while running your code"**, unos 6 minutos después del despliegue. Esto confirmaba que el diagnóstico de la v12.32.28 (que el problema era de tiempo/número de consultas, no de memoria) era incompleto — la base de datos ya no era el cuello de botella, pero algo en la lectura del propio PDF sí estaba agotando la memoria real del proceso, con un PDF que por tamaño (730 KB, 99 páginas) no debería dar ningún problema.

**Causa real — un límite conocido y documentado de la propia librería `pdfplumber`, que el `flush_cache()` de la v12.32.21 no llegaba a cubrir**: `pagina.flush_cache()` solo borra los atributos que pdfplumber declara como "cacheados" en cada `Page` (por ejemplo `_layout`) — pero el objeto `Page` en sí (con el objeto de pdfminer que representa la página completa, y con un `get_textmap` que pdfplumber envuelve en `functools.lru_cache` por cada página) sigue vivo, porque `pdfplumber.PDF.pages` es una propiedad que, la primera vez que se lee (con `for pagina in pdf.pages:`, tal y como hacían las tres funciones de esta app que procesan PDFs con pdfplumber), construye TODAS las páginas del documento de golpe y las guarda en una lista interna (`pdf._pages`) que vive mientras exista el objeto `pdf` — es decir, durante TODA la subida, no solo mientras se procesa cada página. Cada página procesada se queda ahí para siempre, con su tabla ya extraída y su texto ya cacheado en memoria, aunque ya no se necesite para nada más. Medido con un script de perfilado de memoria real (librería `psutil`, no una estimación) sobre el PDF real de 99 páginas / 281 pedidos que reportó Víctor: la memoria del proceso subía de forma lineal, unos 4,2 MB por página, hasta un pico de **497,7 MB** al llegar a la página 99 — con el límite de 512 MB de Render (plan gratuito), esto encaja exactamente con el "Ran out of memory" del panel de Eventos, y explica por qué el fix de la v12.32.28 (que sí era un problema real y sí hacía falta) no bastaba: eran dos problemas independientes, uno de base de datos y otro de memoria, y solo se había corregido el primero.

**Cambio en `app.py`**: en las tres funciones que leen un PDF con `pdfplumber` (`_extraer_listado_detallado_completo()` — Pedidos DETALLADO, `_extraer_albaranes_detallado_completo()` — Albaranes DETALLADO, y `_extraer_albaran_confirmacion_individual()` — confirmación de un albarán suelto), justo después de `pagina.flush_cache()`, se añade una línea más: `pdf.pages[i] = None`, que vacía la propia lista interna de pdfplumber una vez que esa página ya está totalmente procesada y no hace falta conservarla. Es un cambio mínimo (una línea por función) y no toca en ningún caso qué datos se extraen ni cómo se guardan — solo cambia qué se mantiene en memoria mientras se recorre el PDF. Ninguna de las tres funciones vuelve a usar `pdf.pages` después del bucle, así que no hay ningún otro sitio al que afecte.

**Verificación**: `python3 -m py_compile app.py` sin errores. Medido con `psutil` sobre el mismo PDF real de 99 páginas / 281 pedidos que hacía caer el servicio: con el fix, el pico de memoria durante la lectura baja de 497,7 MB a **99,6 MB** (sin ningún `gc.collect()` manual añadido al código de producción — el recolector de basura normal de Python ya libera lo que sobra en cuanto se le quita la referencia). Repetido a escala completa con el listado real de 739 páginas / 2.018 pedidos (el que en su día motivó el `flush_cache()` de la v12.32.21): pico de memoria de solo **152,7 MB**, frente a los varios GB que le habrían hecho falta sin este fix (unos 4,2 MB × 739 páginas). Probado también de extremo a extremo contra un PostgreSQL 16 real (esquema completo, migraciones incluidas): el mismo PDF de 99 páginas / 281 pedidos que falló en producción se procesa correctamente en 35 segundos en local (281 pedidos nuevos, 281 actualizados de departamento, 2.281 líneas guardadas — mismo resultado numérico que antes del fix, como cabía esperar). Las otras dos funciones (`_extraer_albaranes_detallado_completo()` sobre un listado real de Albaranes de 2.422 líneas, y `_extraer_albaran_confirmacion_individual()` sobre una confirmación de albarán suelto real) se probaron por separado tras el cambio y devuelven exactamente el mismo resultado que antes, sin ningún error.

**Revisión de otros documentos (norma 5)**: `GUIA_DESPLIEGUE.md`, `INSTRUCCIONES_RESTAURACION.md`, `PENDIENTES.md`, `docs/hallazgo-seguridad-princess.md` — no aplica. `README.md` sí: versión actual.

**Lección para el futuro**: "confirmado empíricamente que no da OOM" (como decía el comentario original de la v12.32.21 sobre el `flush_cache()`) no es lo mismo que "no puede dar OOM" — el `flush_cache()` de entonces sí resolvió el problema que se estaba probando en ese momento, pero pdfplumber tiene MÁS de un sitio donde retiene memoria por página, y solo se había tapado uno. Cuando una librería de terceros documenta (o, como aquí, cuando su propio código fuente revela) que cachea algo "hasta cerrar el documento", conviene revisar TODA la cadena de objetos que quedan vivos, no solo el primer atributo que aparece en la documentación — y medir con una herramienta de memoria real (no solo con el tiempo que tarda, ni con la RAM disponible en el entorno de pruebas local, que no tiene el límite de 512 MB de Render) antes de dar el problema por cerrado.
...
# v12.32.28 — 4 septiembre 2026

Corregido bug real de la v12.32.27: "Departamentos y líneas (SAP detallado)" reiniciaba el servicio entero incluso con un PDF pequeño — la causa no era memoria, eran casi 4.000 consultas a la base de datos una por una en vez de en bloque

**Qué pasó**: nada más desplegar la v12.32.27, Víctor subió un listado detallado de solo 99 páginas / 281 pedidos (mucho más pequeño que el de 739 páginas que causaba el problema de memoria original) y, al cabo de unos 4 minutos y medio, saltó el mismo aviso de siempre: "El job no existe o ha caducado". El log de Render confirmó que el servicio se reinició entero a mitad de la subida — mismo síntoma que el problema de memoria de la v12.32.21, pero con un PDF que no debería haber dado ningún problema por tamaño.

**Causa real — no era memoria, era volumen de consultas**: `_actualizar_departamentos_desde_listado_detallado()` hacía una consulta a la base de datos por cada cosa que tocaba: el alta nueva de v12.32.27 (una por pedido), el SELECT y el UPDATE del departamento (una y otra por pedido), el UPDATE de `pedidos.departamento_id` (otra más por pedido), el DELETE de líneas (una por pedido) y un INSERT por cada línea de artículo. Para un listado de 281 pedidos con 2.281 líneas, eso son casi 4.000 consultas seguidas — cada una con su propio viaje de ida y vuelta hasta Supabase. Contra un PostgreSQL local (sin red de por medio) esto es instantáneo, por eso no se detectó al probarlo antes de entregar la v12.32.27; contra la base de datos real, con la latencia de red normal de cualquier consulta a Supabase, esas casi 4.000 consultas se acumulan hasta tardar varios minutos — tiempo de sobra para que Render diera por colgado el proceso y lo reiniciase, con el mismo efecto visible de siempre (se pierde el job en memoria, "no existe o ha caducado"), aunque el origen esta vez no fuera un PDF grande sino miles de consultas innecesarias donde bastaban unas pocas.

**Cambio en `app.py`**: la misma función se reescribe para hacer todo por bloques en vez de pedido a pedido, usando `psycopg2.extras.execute_values` — el alta de pedidos nuevos en `sap_pedidos_listado` es ahora un único INSERT con todos los pedidos a la vez (`ON CONFLICT DO NOTHING`, igual que antes), la actualización de departamento es un único UPDATE (con una tabla temporal `VALUES`) para todos los pedidos con departamento, la propagación a `pedidos.departamento_id` es otro único UPDATE en bloque, y las líneas se borran con un único DELETE (`pedido_num_sap = ANY(...)`) y se insertan en unos pocos INSERT paginados de 1.000 en 1.000 en vez de uno por línea. El resultado numérico que ve el usuario es idéntico (mismos contadores, mismo criterio de qué se actualiza), solo cambia cuántas veces se habla con la base de datos: de casi 4.000 consultas a poco más de una decena. De paso, se añade una defensa menor: cualquier pedido cuyo número saliera vacío del PDF (no debería pasar, pero antes habría reventado el INSERT en bloque contra una columna NOT NULL) se descarta silenciosamente en vez de romper toda la subida.

**Verificación**: `python3 -m py_compile app.py` sin errores. Probado de extremo a extremo contra un PostgreSQL 16 real con el esquema completo de la app: con el mismo PDF real de 99 páginas / 281 pedidos que reportó Víctor, el resultado es idéntico al que debía dar la v12.32.27 (281 pedidos nuevos registrados, 281 actualizados de departamento, 2.281 líneas guardadas) pero con un pedido ya dado de alta manualmente (sin departamento) confirmando que la propagación a `pedidos.departamento_id` sigue funcionando igual en bloque que antes uno a uno; segunda subida del mismo PDF idempotente (0 pedidos nuevos, mismo total de filas y líneas, sin duplicados). No se ha podido medir el tiempo real contra Supabase desde aquí (sin acceso a esa base de datos ni a esa red) — al desplegar, conviene repetir la subida del mismo PDF que falló y confirmar que ahora termina en segundos, no en minutos.

**Revisión de otros documentos (norma 5)**: `GUIA_DESPLIEGUE.md`, `INSTRUCCIONES_RESTAURACION.md`, `PENDIENTES.md`, `docs/hallazgo-seguridad-princess.md` — no aplica. `README.md` sí: versión actual.

**Lección para el futuro**: cualquier función que recorra un PDF/listado línea a línea o pedido a pedido y haga una consulta a la base de datos DENTRO del bucle es un riesgo de este mismo tipo, aunque funcione perfectamente en local — la diferencia solo aparece contra una base de datos remota de verdad, con latencia de red real. Antes de dar por buena una función así, conviene contar cuántas consultas hace en el peor caso (nº de pedidos × nº de pasos) y, si el número es alto, escribirla desde el principio en bloque.

**Entrega**: `app.py`, `templates/index.html` (solo versión), `README.md`, más este changelog/`docs/HISTORIAL_CAMBIOS.md`. `models.py` y `requirements.txt` no cambian.

---

# v12.32.27 — 4 septiembre 2026

Pieza 1+2 del rediseño "solo con los dos listados detallados": el listado DETALLADO de Pedidos ya puede registrar y ofrecer para crear los pedidos que el RESUMIDO nunca llegó a traer

**Contexto — por qué esta entrega**: Víctor planteó que quiere poder trabajar únicamente con los dos listados DETALLADO de SAP (Pedidos y Albaranes), sin depender de subir también el listado RESUMIDO ni de compararlos a la vez ("YO PENSABA QUE CON LOS DOS LISTADOS DETALLADOS PEDIDOS Y ALBARANES YA PODIAMOS HACER TODO EL TRABAJO... ¿NO ES POSIBLE?"). Tras confirmar con él la viabilidad (proveedor+fecha se extraen de forma fiable del detallado, 100% sobre 2.018 cabeceras reales) se acordó entregarlo por piezas, empezando por la más básica: que un pedido visible SOLO en el listado detallado deje de ser invisible para la app.

**Qué hacía antes y por qué era un problema**: `_extraer_listado_detallado_completo()` solo guardaba, de la cabecera de cada pedido ("NNNNNNNN - Pedido DD/MM/AAAA HH:MM:SS (PROVEEDOR Teléfono:...)"), el número de pedido — proveedor y fecha se descartaban. Y `_actualizar_departamentos_desde_listado_detallado()`, al procesar ese listado, si un pedido no tenía ya una fila en `sap_pedidos_listado` (creada por una subida previa del RESUMIDO), simplemente lo contaba como "no encontrado" y no hacía nada más con él — ni su departamento, ni (lo más importante) su existencia quedaban registrados. En la práctica: un pedido que solo aparecía en el detallado (porque nunca se subió o no se subió a tiempo el resumido con ese rango de fechas) nunca aparecía en "Comparar listado PDF" ni se podía dar de alta con "Crear pedidos desde SAP" — dependía por completo del RESUMIDO para existir en la app.

**Cambio en `app.py`**:
- `_extraer_listado_detallado_completo()`: ahora también extrae, de la propia cabecera, `proveedor_raw` (recortado antes de "Teléfono:", que siempre aparece pegado sin separador — verificado sobre las 2.018 cabeceras reales del listado de Víctor, 100% con ese formato exacto) y `fecha_hora_fecha`/`fecha_hora_hora` (la fecha/hora de la propia cabecera "Pedido DD/MM/AAAA HH:MM:SS" — el mismo dato que ya guarda `_PATRON_LISTADO_SIMPLIFICADO` del RESUMIDO bajo esos mismos nombres de columna).
- `_actualizar_departamentos_desde_listado_detallado()`: por cada pedido del PDF detallado, antes de la lógica ya existente de departamento, se inserta (si no existe ya) una fila en `sap_pedidos_listado` con ese proveedor, esa fecha/hora de cabecera, y la fecha de pedido/fecha de entrega de su primera línea de artículo (mismos campos que ya guarda `sap_pedidos_lineas` desde v12.32.18) — `ON CONFLICT (hotel_id, pedido_num_sap) DO NOTHING`, así que **nunca toca una fila que ya exista** (si el RESUMIDO ya trajo ese pedido, aunque solo fuera una vez, esta alta no hace nada). El importe (base/recibido/pendiente) y el estado SAP se dejan siempre en NULL — el detallado no los trae, y sumar líneas no reproduce siempre al céntimo el importe real de SAP (motivo ya documentado junto a `_PATRON_LISTADO_SIMPLIFICADO`); rellenarlos de forma aproximada para pedidos antiguos es la Pieza 3 de este mismo rediseño, todavía pendiente. Nuevo campo en el resultado: `pedidos_nuevos_en_listado_sap`.

**Qué NO cambia**: `_pedidos_sap_no_registrados()` y `crear_pedidos_desde_sap()` — la función que decide qué pedidos ofrecer para crear y la que los crea de verdad — no se han tocado en absoluto. Como ambas ya trabajan exclusivamente contra `sap_pedidos_listado` (sin importar si una fila vino del RESUMIDO o, desde ahora, del DETALLADO), un pedido registrado por esta vía nueva ya aparece automáticamente en "Comparar listado PDF" y es seleccionable en "Crear pedidos desde SAP", con estado inicial ENVIADO AL PROVEEDOR (al no haber importe, `_entrega_estado()` lo resuelve como "No entregado" — nunca se inventa un estado de entrega sin datos reales, mismo criterio que corrigió el bug real de v12.32.13).

**Cambio en `templates/index.html`**: en el resultado de "🏷️ Departamentos y líneas (SAP detallado)", nueva píldora morada "N pedido(s) nuevo(s) registrado(s) desde el detallado (sin resumido)" y su aviso explicando que su importe queda vacío hasta subir el resumido correspondiente (o hasta la próxima entrega, que lo aproximará). Se retira el aviso antiguo de "sin listado resumido todavía" invitando a subirlo — ya no aplica, porque ahora ese hueco se rellena solo desde el propio detallado.

**Verificación**: `python3 -m py_compile app.py` sin errores; los 8 bloques `<script>` de `templates/index.html` (extraídos con un parser HTML real, no con una regex ingenua que también capturaba la palabra "script" dentro de comentarios) pasan `node --check` sin errores. Extracción de proveedor/fecha comprobada contra las 739 páginas completas del listado real de tres meses de Víctor: 2.018 cabeceras, 100% con "Teléfono:" presente, 0 proveedores vacíos tras el recorte. Probado de extremo a extremo contra un PostgreSQL 16 real (con el esquema completo de la app, migraciones incluidas, no una tabla simulada): (1) subiendo ese PDF real a un hotel con una fila ya existente en `sap_pedidos_listado` (simulando un pedido que el RESUMIDO ya había traído, con su importe real), esa fila sale intacta — proveedor e importe originales sin tocar; (2) se registran 2.017 pedidos nuevos (2.018 del PDF menos el ya existente); (3) `_pedidos_sap_no_registrados()` los devuelve correctamente como candidatos a crear, con proveedor/fecha de pedido/fecha de entrega bien resueltos; (4) se creó un pedido real de principio a fin a partir de uno de esos candidatos, sin ninguna excepción, con `fecha_tramitacion`/`fecha_entrega_especifica` correctas y `total_pedido = 0,00` (a la espera de la Pieza 3); (5) subir el mismo PDF una segunda vez es idempotente — 0 pedidos nuevos, ninguna fila duplicada.

**Revisión de otros documentos (norma 5)**: `GUIA_DESPLIEGUE.md`, `INSTRUCCIONES_RESTAURACION.md`, `PENDIENTES.md`, `docs/hallazgo-seguridad-princess.md` — no aplica. `README.md` sí: versión actual (el bullet de "Comparar Pedidos + Albaranes (SAP)" se actualizará cuando el rediseño esté más completo, para no describir un flujo todavía a medias).

**Sigue pendiente**: Pieza 3 (importe aproximado para pedidos antiguos sumando líneas del detallado, marcado explícitamente como "aproximado" para no confundirlo con el importe exacto del RESUMIDO), Pieza 4 (cruce Pedidos↔Albaranes detallado para proponer el estado de entrega) y Pieza 5 (unificar los 5 botones actuales en un flujo de dos pasos) — entregas siguientes, una a una, según lo acordado con Víctor.
...
**Aviso de Víctor**: pegó 3 filas del linter de seguridad de Supabase ("RLS Disabled in Public", nivel ERROR) para `sap_pedidos_lineas`, `sap_pedidos_listado` y `sap_albaranes_lineas`.

**Alcance real, más allá de lo reportado**: `_auto_migrate()` ya llevaba desde agosto una lista de tablas a las que activa RLS al arrancar (`egress_tracking`, `db_size_tracking`, etc. — ver comentario junto a esa lista, incidente real documentado entonces), con el mismo razonamiento que aplica aquí: esta app **nunca** usa la API REST automática de Supabase (PostgREST) — todo pasa por conexión directa a Postgres con `DATABASE_URL` —, así que activar RLS sin ninguna política no rompe nada y solo cierra un acceso público que no debería existir. El problema es que esa lista se mantenía a mano, nombre a nombre, y cada tabla nueva se quedaba fuera hasta que alguien se acordaba de añadirla — así se quedaron fuera las 3 que reportó Víctor. Al revisar el alcance completo se encontraron **21 tablas más creadas en `app.py`** (`pedido_adjuntos`, `emails_sistema_pendientes`, `usuario_hoteles`, `whatsapp_log`, `familias`, `config_alertas`... la lista completa en el propio commit) y, más importante, **las tablas originales del esquema** definidas en `models.py` — `pedidos`, `usuarios`, `proveedores`, `hoteles`, `historial_estados`, `departamentos`, `pedidos_eliminados`, `emails_log`, entre otras — en la misma situación desde el origen del proyecto: nunca han tenido RLS activado. Es muy probable que el Security Advisor de Supabase esté marcando bastantes más errores de los 3 que se pegaron aquí.

**Decisión**: en vez de seguir ampliando una lista fija cada vez que aparece una tabla nueva sin RLS (con el riesgo de volver a olvidar alguna, como ya pasó), se sustituye por una consulta que detecta dinámicamente TODA tabla del esquema `public` sin RLS activado y se lo activa — sin tener que nombrarlas, y sin que vuelva a hacer falta acordarse de añadir las que se creen en el futuro.

**Cambio en `app.py`**: en `_auto_migrate()`, el bloque que antes era `for _tabla_rls in (lista fija de nombres):` se sustituye por una consulta a `pg_class`/`pg_namespace` (`WHERE n.nspname = 'public' AND c.relkind = 'r' AND NOT c.relrowsecurity`) que devuelve exactamente las tablas que aún no tienen RLS, y un `ALTER TABLE IF EXISTS "<tabla>" ENABLE ROW LEVEL SECURITY` por cada una — mismo sitio (al principio de `_auto_migrate()`, antes de las más de 100 sentencias restantes, por el mismo motivo que ya se explicaba ahí: que se aplique siempre, pase lo que pase más abajo esa misma ejecución) y mismo criterio de seguridad (sin ninguna política, no cambia qué puede leer o escribir la propia app, que se conecta como propietaria de las tablas). Es idempotente: en cada arranque solo actúa sobre las tablas que en ese momento sigan sin RLS, así que no repite trabajo ni falla al re-ejecutarse sobre las que ya están activadas.

**Verificación**: `python3 -m py_compile app.py` sin errores. Probado de extremo a extremo contra un PostgreSQL 16 real y aislado (no la base de producción, no hay acceso a ella desde aquí): (1) la consulta detecta correctamente solo las tablas sin RLS y ninguna de las que ya lo tenían; (2) el `ALTER TABLE` funciona y es idempotente (repetirlo sobre una tabla ya activada no da error); (3) con RLS activado y sin política, un rol sin privilegios de propietario (equivalente al rol `anon`/`authenticated` que usaría PostgREST) deja de ver ninguna fila aunque tenga permiso `SELECT` concedido; (4) el propio propietario de la tabla (equivalente al rol con el que esta app se conecta vía `DATABASE_URL`) sigue viendo y pudiendo modificar todas las filas con total normalidad — confirma que el cambio no afecta en nada al funcionamiento real de la app. No se ha podido ejecutar contra la base de producción de Supabase desde aquí — al desplegar, conviene volver a correr el Security Advisor de Supabase (Database → Security Advisor) y comprobar que el aviso "RLS Disabled in Public" desaparece para todas las tablas, no solo las 3 originales.

**Revisión de otros documentos (norma 5)**: `GUIA_DESPLIEGUE.md`, `INSTRUCCIONES_RESTAURACION.md`, `PENDIENTES.md`, `docs/hallazgo-seguridad-princess.md` — no aplica (no cambia nada del despliegue ni de cómo se conecta la app; el hallazgo de seguridad ya documentado en agosto para las primeras tablas sigue siendo la referencia, solo se amplía su alcance). `README.md` sí: versión actual.

**Sigue pendiente**: confirmar en el Security Advisor de Supabase, tras desplegar, que no queda ninguna tabla marcada; valorar si en algún momento interesa además definir políticas de RLS reales (no solo "activado sin política", que ya de por sí bloquea el acceso público) para algún caso concreto, aunque a día de hoy no hace falta porque nada usa PostgREST.

**Entrega**: `app.py`, `templates/index.html` (solo versión), `README.md`, más este changelog/`docs/HISTORIAL_CAMBIOS.md`. `GUIA_DESPLIEGUE.md`, `models.py` y `requirements.txt` no cambian.
...
**Contexto**: tras entregar v12.32.19 (sugerencias por proveedor+código de artículo), Víctor compartió dos datos nuevos. Primero, un PDF ("97659.pdf") que demuestra que el programa de almacén SÍ puede imprimir, para UN albarán a la vez, un volcado con el pedido asociado en el campo "Pedido/s" — "CUANDO UN ALBARAN NOS DE DUDAS, SIEMPRE EXISTE LA OPCION DE ENTREGAR ESTE TIPO DE PDF... EL PROBLEMA ES QUE EL PROGRAMA DE ALMACEN NO DEJA IMPRIMIR POR MESES O PROVEEDORES, SOLO EL ALBARAN QUE TENGA EN PANTALLA, POR ESO SERIA PARA UN CASO PUNTUAL". Segundo, que en Pedidos ya existe desde hace tiempo un campo donde, al pasar un pedido a Entrega parcial/Total, se anota a mano el número de albarán y su base imponible (`entrada_albaran_num`) — "DATOS QUE POSIBLEMENTE NOS VENGAN TAMBIEN BIEN PARA LA ASOCIACION". Preguntado qué número es exactamente ese, Víctor confirmó que es "el número de asiento DALI/SAP" (p. ej. "00098989" en "Albarán: 00098989 - ALV2813635"), el mismo identificador que ya se guarda como `sap_albaranes_lineas.albaran_id` — con o sin ceros a la izquierda. Con esa confirmación, ambas fuentes son asociaciones DIRECTAS (no una sugerencia por proveedor+artículo) y pasan a mostrarse como "confirmado" por delante de las sugerencias de v12.32.19.

**Cambio en `app.py`**:
- `sap_albaranes_lineas` gana la columna `pedido_num_sap_confirmado` (con `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` para bases ya desplegadas en v12.32.19) y dos índices nuevos, por `(hotel_id, albaran_id)` y `(hotel_id, pedido_num_sap_confirmado)`.
- `_extraer_albaran_confirmacion_individual()`: parser nuevo (pdfplumber) del PDF de confirmación de UN albarán suelto — distingue la tabla de metadatos (clave/valor: Código, Proveedor, Albarán Proveedor, Pedido/s, Fecha Albarán) de la tabla de líneas de artículo, y extrae el/los número(s) de pedido del campo "Pedido/s" (formato "NNNNNNNN - Pedido DD/MM/AAAA HH:MM:SS"). Verificado estructuralmente contra el PDF real de Víctor (97659.pdf, 15 líneas).
- `_importar_albaran_confirmacion()`: guarda las líneas de ese PDF en `sap_albaranes_lineas` con `pedido_num_sap_confirmado` relleno (reemplaza cualquier versión anterior guardada de ese mismo `albaran_id`, venga del listado en bloque o de una subida anterior de este mismo PDF). Si el PDF trae más de un pedido en "Pedido/s" (caso no visto todavía con datos reales), se usa el primero y se avisa en el resultado — no se ha necesitado resolver ese caso porque no ha aparecido.
- `_sugerencias_albaran_pedido()` reescrita: además de las sugerencias por proveedor+código de v12.32.19, calcula ahora un tier `confirmados` a partir de (a) filas de `sap_albaranes_lineas` con `pedido_num_sap_confirmado` que coincide (normalizando ceros a la izquierda con `_normalizar_pedido_num()`) con el pedido consultado, y (b) los números de albarán ya registrados a mano en `pedidos.entrada_albaran_num` (parseados con `_parse_albaran_entries()`, ya existente) comparados por `_normalizar_num_albaran()` contra los `albaran_id` disponibles. Cada candidato de la tabla por línea gana un flag `confirmado`; los confirmados se ordenan primero y una línea deja de marcarse "ambigua" en cuanto tiene un candidato confirmado.
- Endpoint nuevo `POST /api/albaranes/importar-confirmacion` (admin, síncrono — el PDF es de una sola página, no hace falta job en segundo plano).

**Cambio en `templates/index.html`**: tercer botón junto a los de Albaranes — "📎 Confirmar un albarán suelto" (admin, gateado en los dos sitios habituales), con su modal (hotel + PDF, envío directo). "🔍 Sugerencias de Albarán" ahora muestra, antes de la tabla por línea, un bloque "🔒 Albarán(es) confirmado(s)" cuando los hay (con su fuente: PDF de confirmación o registrado en el pedido), y marca con 🔒 en vez de ✅/❔ los candidatos ya confirmados dentro de la tabla.

**Verificación**: `python3 -m py_compile app.py` sin errores; `node --check` sobre los 8 bloques `<script>` de `templates/index.html` (extraídos con un parser HTML real) sin errores. El parser del PDF de confirmación se verificó estructuralmente inspeccionando directamente con pdfplumber el PDF real que compartió Víctor (tabla de metadatos y tabla de líneas con las columnas exactas que usa el código), pero — a diferencia de los parsers de v12.32.18/19 — no se ha ejecutado la función completa en aislado contra ese PDF antes de esta entrega. **No probado en vivo contra producción**. Al desplegar, conviene: (1) subir el PDF real de un albarán suelto con "Confirmar un albarán suelto" y comprobar que el resumen (albarán, pedido SAP, líneas guardadas) coincide con lo esperado; (2) para un pedido que ya tenga `entrada_albaran_num` Y albaranes importados (listado en bloque o PDF suelto) para ese hotel, comprobar en "Sugerencias de Albarán" que aparece el bloque de confirmados; (3) si en algún momento aparece un PDF con más de un pedido en "Pedido/s", revisar a mano ese caso — el aviso en el resultado de la importación lo señala, pero solo se guarda asociado al primero.

**Revisión de otros documentos (norma 5)**: `GUIA_DESPLIEGUE.md`, `INSTRUCCIONES_RESTAURACION.md`, `PENDIENTES.md`, `docs/hallazgo-seguridad-princess.md` — no aplica (mismo patrón de tabla/endpoint de solo lectura que v12.32.19, sin cambios de despliegue, seguridad ni credenciales). `README.md` sí: versión actual.

**Sigue pendiente**: el caso de un albarán asociado a más de un pedido en el PDF de confirmación (nunca visto con datos reales); validar en uso real que el tier "confirmado" por `entrada_albaran_num` funciona bien cuando ese campo tiene varias entradas para el mismo pedido; y, como antes, decidir si estas sugerencias se integran también dentro de la ficha del pedido una vez validadas en uso real.

**Entrega**: `app.py`, `templates/index.html`, `README.md`, más este changelog/`docs/HISTORIAL_CAMBIOS.md`. `models.py` y `requirements.txt` no cambian.

---

# v12.32.19 — 4 septiembre 2026

Primera entrega del proyecto de cruce Pedidos↔Albaranes: importar el Listado de Albaranes de SAP y pedir sugerencias (no vinculantes) de con qué albarán se corresponde cada línea de un pedido

**Contexto — retomado en esta sesión**: Víctor propuso usar la columna de cantidad pendiente del listado detallado para decidir el estado de entrega sin depender del resumido, y por separado preguntó si se podía cruzar Pedidos con el nuevo Listado de Albaranes de SAP (que adjuntó como ejemplo) para identificar qué albarán corresponde a qué pedido — el propio programa de almacén no guarda esa relación. Antes de tocar código se hicieron pruebas empíricas con datos reales de varios meses (ver detalle en el hilo — no se repite aquí por no ser cambio de código), con un hallazgo importante a mitad de camino: las primeras pruebas mezclaban proveedores de alimentación/bebida/limpieza, que Víctor confirmó que quedan FUERA del alcance de esta app ("está pensada para pedidos NO alimentación y bebidas" / "limpieza tampoco entra"). Repetidas las pruebas usando como filtro los proveedores realmente marcados `sujeto_seguimiento=TRUE` en el catálogo (en vez de adivinar por familia SAP) y con una ventana más amplia (pedidos oct'25–mar'26 contra albaranes dic'25–may'26, ambos aportados por Víctor), el cruce por proveedor+código de artículo salió con 100% de cobertura, solo 17% de combinaciones ambiguas (media 2,4 candidatos) y 95,5% de coincidencia exacta en cantidad — base suficiente para construir una primera pantalla de sugerencias. La idea de clasificar el estado de entrega solo con la cantidad pendiente del detallado, en cambio, no encontró datos suficientes en el alcance real de la app (muestra de solo 18-55 pedidos según el filtro) para poder recomendarla todavía — queda pendiente de más meses de listado resumido si se quiere retomar.

De camino, Víctor pidió también un script SQL para identificar los proveedores `sujeto_seguimiento` (no es cambio de `app.py`, resuelto directamente por chat) y, tras subir su propia exportación de códigos DALI, un cruce por nombre para rellenar los que le faltaban en la tabla `proveedores` (45 de 46, ejecutado por Víctor mismo en Supabase; 1 sin encontrar en su exportación, pendiente de que él lo revise).

**Cambio en `app.py`**:
- Nueva tabla `sap_albaranes_lineas` (`hotel_id`, `periodo_desde`, `periodo_hasta`, `proveedor_codigo`, `proveedor_nombre`, `albaran_id`, `albaran_ref`, `familia_codigo`, `familia_nombre`, `codigo_articulo`, `descripcion`, `unidad`, `formato`, `cantidad_txt`, `precio_txt`, `descuento_pct_txt`, `importe_txt`) con índice por `(hotel_id, codigo_articulo)` — guarda tal cual el Listado de Albaranes de Compra de SAP (jerárquico: Proveedor → Albarán → Familia → Subfamilia → Artículo), confirmado que NO trae el número de pedido SAP en ningún sitio.
- `_extraer_albaranes_detallado_completo()`: nuevo parser (pdfplumber) de ese listado — extrae también el periodo cubierto de la cabecera del PDF ("Período desde ... hasta ...") para poder reemplazar por periodo en cada subida sin duplicar filas.
- `_importar_albaranes_listado()`: guarda las líneas extraídas — si reconoce el periodo de la cabecera, borra antes las filas de ese mismo hotel+periodo (subida idempotente); si no lo reconoce, inserta sin borrar nada y lo avisa en el resultado, para no arriesgarse a borrar datos de otro periodo por error.
- `_sugerencias_albaran_pedido()`: para un pedido dado, cruza sus líneas ya guardadas en `sap_pedidos_lineas` (v12.32.18) contra `sap_albaranes_lineas`, por proveedor (normalizado con `_normalizar_nombre_proveedor()`, ya existente desde 2026-08-06 — se reutiliza en vez de duplicar el criterio) + código de artículo. Solo aplica si el proveedor del pedido está marcado `sujeto_seguimiento` en el catálogo; si no, explica por qué no aplica en vez de devolver un resultado vacío sin más. Marca cada candidato como coincidencia exacta o no (por cantidad) y cada línea como ambigua si hay más de un candidato — es una sugerencia para revisar a mano, nunca escribe nada en el pedido ni cambia su estado.
- Endpoints nuevos, ambos de solo lectura frente al resto de la app: `POST /api/albaranes/importar-listado` (+ `GET .../<job_id>`, admin, mismo patrón de job en segundo plano que "Departamentos y líneas") y `GET /api/pedidos/<id>/sugerencias-albaran` (cualquier usuario logueado).

**Cambio en `templates/index.html`**: dos botones nuevos junto a "Departamentos y líneas" (admin, gateados en los dos sitios habituales) — "📥 Importar Albaranes (SAP detallado)" (mismo patrón de modal+job que ya existía) y "🔍 Sugerencias de Albarán" (modal simple: escribe el nº de pedido SAP, resuelve el pedido vía `/api/pedidos?q=...` y muestra la tabla de líneas con sus candidatos, marcando ✅ coincidencia exacta / ❔ candidato dudoso / fila resaltada si hay más de uno).

**Verificación**: `python3 -m py_compile app.py` sin errores; `node --check` sobre los 9 bloques `<script>` de `templates/index.html` (extraídos con un parser HTML real, no con una regex ingenua) sin errores. El parser `_extraer_albaranes_detallado_completo()` se probó de forma aislada (fuera de la app, exec del código fuente) contra un listado real de mayo (263 páginas): 4.959 líneas, periodo detectado correctamente (01/05/2026 – 30/05/2026). La lógica de cruce por proveedor+código es la misma validada empíricamente antes de escribir el código (ver contexto arriba). **No se ha podido probar en vivo contra producción** — no hay conexión a la base de datos desde aquí. Al desplegar: (1) sube un Listado de Albaranes real con "Importar Albaranes" y comprueba el resumen (líneas importadas, periodo reconocido); (2) prueba "Sugerencias de Albarán" con un pedido conocido de un proveedor `sujeto_seguimiento` que ya tenga líneas guardadas (p. ej. uno de CASA DELFIN de marzo) y comprueba que las sugerencias tienen sentido antes de confiar en la herramienta para más pedidos.
...
**Sigue pendiente**: el 1 proveedor sin código DALI encontrado (ALL SPORT ALTERNATIVAS DEPORTIVAS S); ampliar la prueba de clasificación por cantidad pendiente con más meses de listado resumido si Víctor quiere retomarla; y, más adelante, decidir si las sugerencias de este cruce se integran también dentro de la ficha de un pedido (en vez de solo en esta herramienta aparte) una vez validada en uso real.
...
**Petición de Víctor**, tras desplegar v12.32.16 y antes de empezar a subir quincenas del listado detallado desde el 01-01-2026 para "actualizar totalmente el flujo anual": preguntó (1) si el orden de subida de las quincenas importa, o si primero debía cubrir los periodos de los pedidos ya creados automáticamente desde SAP para que el departamento se rellenara bien, y (2) pidió poder pausar avisos/correos internos/correos a proveedores mientras dura la actualización masiva, porque "ahora que voy a actualizar departamentos, comenzarán a saltar correos... con pedidos entregados etc".

**Sobre el orden de subida**: no importa — `_actualizar_departamentos_desde_listado_detallado()` empareja cada pedido del listado detallado por su número, no por fecha, así que el resultado final es el mismo suba las quincenas en el orden que sea. Lo único que de verdad hace falta es que el listado **resumido** (simplificado) ya tenga guardada una fila para ese pedido (`sap_pedidos_listado`, vía "Comparar listado PDF") — algo que ya se cumple siempre para los pedidos creados automáticamente desde SAP, porque esa creación se hace precisamente a partir de esa misma tabla. Si para algún pedido creado a mano el resumido de ese periodo todavía no se ha subido, el resultado de la actualización lo avisa aparte ("sin listado resumido todavía") y basta con repetir esa subida del detallado más adelante.

**Hallazgo al revisar esta pregunta — corregido en la misma entrega**: `_actualizar_departamentos_desde_listado_detallado()` (v12.32.16) solo rellenaba `sap_pedidos_listado.departamento_sap_codigo`; el departamento del PEDIDO ya dado de alta en la app (`pedidos.departamento_id`) solo se completaba en el backfill retroactivo de `_auto_migrate()`, que corre una única vez por arranque/despliegue de la app. Con el plan de Víctor de ir subiendo muchas quincenas sueltas a lo largo del tiempo, eso habría significado que el departamento de los pedidos ya creados solo se aplicaría de verdad tras el SIGUIENTE despliegue, no al subir cada listado. **Cambio en `app.py` — `_actualizar_departamentos_desde_listado_detallado()`**: ahora, justo después de guardar el código en `sap_pedidos_listado`, si el pedido correspondiente ya existe en `pedidos` y todavía no tiene departamento asignado, se lo asigna en el momento (mismo criterio de siempre: nunca se pisa uno ya puesto, a mano o antes; es un simple `UPDATE` de un campo informativo que no cambia `estado` ni `fecha_tramitacion`, así que no dispara ningún email ni aviso). El resumen de la subida ahora informa también cuántos pedidos ya dados de alta se completaron así.

**Sobre la pausa de avisos**: no existía ningún interruptor global — `activar_reclamacion_proveedor_auto` (Config Alertas) ya permitía desactivar los emails automáticos (reclamación a proveedor + avisos de firma pendiente), pero el aviso por Telegram a los compradores del job diario seguía disparándose pasase lo que pasase con ese interruptor. **Cambio en `app.py`**: nueva clave `pausa_avisos_automaticos` en `config_alertas` (grupo "🌐 Umbrales globales", visible y editable en Administrador → Parámetros de alertas sin ningún cambio de interfaz — el panel ya lee la tabla de forma genérica), comprobada al principio mismo de `_job_alertas_diarias_inner()`: si está activa, el job entero se corta ahí, sin evaluar ni enviar nada — ni Telegram a compradores, ni reclamación automática por email a proveedores, ni aviso de firma pendiente (los tres salen de ese único job). Al reactivarlo no hay nada que "recuperar": el job vuelve a evaluar el estado real de cada pedido en ese momento, exactamente igual que si hubiera seguido corriendo pero sin que nada calificara para aviso mientras tanto.

**Verificación**: `python3 -m py_compile app.py` sin errores; `node --check` sobre los bloques `<script>` de `templates/index.html` sin errores. Repasada a mano la lógica de propagación a `pedidos.departamento_id` (mismo cruce por `pedido_num` sin normalizar que ya usa el backfill de `_auto_migrate()`, mismo criterio "solo si está vacío"). No se ha podido probar en vivo contra producción desde aquí — al desplegar y antes de subir el primer listado detallado histórico, conviene: (1) activar "🚨 PAUSAR..." en Parámetros de alertas; (2) subir las quincenas (en el orden que sea); (3) revisar el resumen de cada subida (departamentos actualizados / pedidos completados); (4) una vez completado el histórico hasta el 31-08-2026, desactivar de nuevo la pausa.
